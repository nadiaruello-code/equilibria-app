...
export async function GET(req: Request) {
  try {
    ...
    if (!canAccessDay(plan, day)) {
      return NextResponse.json({ error: 'Accès réservé au Voyage Complet.' }, { status: 403 });
    }
    ...
    return NextResponse.json({
      error: `Audio introuvable. Ajoute jour-${day}.m4a ou jour-${day}.mp3 dans Supabase Storage.`
    }, { status: 404 });
