...
export async function POST(req: Request) {
  ...
  if (!user || !isAdminEmail(user.email)) return NextResponse.json({ error: 'Accès admin refusé.' }, { status: 403 });
  ...
  try {
    ...
    if (!isM4a && !isMp3) return NextResponse.json({ error: 'Format accepté : .m4a ou .mp3' }, { status: 400 });
